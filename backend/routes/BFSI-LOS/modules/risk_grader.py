...
# Fixed weights
FS_WEIGHT = 50.0        # Financial Strength (sum of 13 equal weights)
...
# Each ratio contributes exactly 50/13 if it PASSES, else 0
...
def compute_risk(
    ...
    """
    Financial Strength: sum of 13 ratios, each = 50/13 if red_flag == False else 0.
    Management Quality: fixed 30
    Industry Risk: fixed 20
    ...
    # Bucket heuristic (keep your original bands)
    ...
    # 🔹 Always save to "risk_rating.json" in extractions folder
    out_path = os.path.join(os.path.dirname(extracted_values_path), "risk_rating.json")
